...
<div class="space-y-8">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-8 mb-10">
        ...
        @foreach($roles as $r)
        <div class="role-card relative bg-white p-8 rounded-[2rem] border border-gray-50 shadow-sm {{ $r['glow'] }} group overflow-hidden">
            ...
            <div class="relative z-10 flex justify-between items-start">
                ...
                <div class="w-12 h-12 rounded-2xl flex items-center justify-center bg-white shadow-inner border border-gray-50 group-hover:rotate-12 transition-transform">
            ...
            <table class="w-full text-left">
                ...
                <tbody class="divide-y divide-gray-50 text-[11px]">
                    ...
                    <tr class="hover:bg-slate-50/80 transition-colors group">
                        <td class="px-8 py-6">
                            <div class="flex items-center gap-4">
                                <div class="w-10 h-10 rounded-xl bg-[#1E3A8A] text-white flex items-center justify-center font-black shadow-lg">
                                    {{ substr($u['name'], 0, 2) }}
                                </div>
                                <div>
                        ...
                        <td class="px-8 py-6 text-center">
                            <span class="inline-block px-4 py-1.5 {{ $u['bg'] }} text-{{ $u['color'] }} rounded-lg font-black uppercase tracking-tighter border border-current opacity-80">
                                {{ $u['role'] }}
                            </span>
                        </td>
                        <td class="px-8 py-6">
                            <div class="flex flex-wrap justify-center gap-2">
                                @foreach($u['mods'] as $m)
                                <span class="px-3 py-1 bg-slate-100 text-slate-500 rounded-md font-black text-[8px] border border-slate-200">{{ $m }}</span>
                                @endforeach
                            </div>
                        </td>
                        <td class="px-8 py-6">
                            <div class="flex justify-end gap-2">
                                <button class="p-2.5 bg-white border border-gray-100 rounded-xl text-slate-400 hover:text-blue-600 hover:shadow-md transition-all">
                                ...
                                <button class="p-2.5 bg-white border border-gray-100 rounded-xl text-slate-400 hover:text-red-600 hover:shadow-md transition-all">
